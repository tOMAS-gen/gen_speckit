...
```powershell
python .specify/scripts/python/get_parallel_groups.py --tasks-path specs/<feature>/tasks.md
```
...
| `cuota_agotada` | `python .specify/scripts/python/update_quota.py --cli <cli> --estado agotada --models-path .specify/models.json`; reasignar la tarea al SIGUIENTE candidato de `asignacion.<nivel>` (luego niveles superiores); actualizar la etiqueta `[M:]` en `tasks.md`; registrar el evento; re-despachar |
