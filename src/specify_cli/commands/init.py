...
def _resolve_skills_agents(skills_opt, selected_ai: str) -> list[str]:
    """Resuelve a qué agente(s) se instalan las skills multi-CLI (gen_speckit).

    ``skills_opt``: valor de la opción ``--skills`` (``None`` = por defecto), que puede ser
    ``"todos"``, un CLI (``claude``/``codex``/``kimi``) o una lista separada por comas.
    Por defecto (``None``/vacío) se usa el mismo agente elegido para las skills base
    (``selected_ai``). Ampliado por T017.
    """
    if not skills_opt:
        return [selected_ai]
    value = str(skills_opt).strip().lower()
    if value == "todos":
...
def register(app: typer.Typer) -> None:
    ...
    def init(
        ...
        skills: str = typer.Option(
            None,
            "--skills",
            help="gen_speckit: agente(s) destino de las skills multi-CLI (claude|codex|kimi|todos o lista separada por comas). Por defecto, el mismo de --integration.",
        ),
        ...
        for key, label in [
            ...
            ("multicli", "Install multi-CLI product (gen_speckit)"),
            ("final", "Finalize"),
        ]:
        ...
        with Live(
            ...
            try:
                ...
                # gen_speckit: instalar el producto multi-CLI (aditivo) DESPUÉS del
                # scaffolding base — así `specify init` entrega base + producto en un solo
                # paso. No es fatal: si algo falla, la base ya quedó lista.
                tracker.start("multicli")
                try:
                    from ..gen_multicli import install_product

                    created = install_product(
                        project_path, _resolve_skills_agents(skills, selected_ai)
                    )
                    tracker.complete("multicli", f"{len(created)} archivos")
                except Exception as multicli_err:
                    tracker.error("multicli", str(multicli_err))
                    _print_cli_warning(
                        "install",
                        "multicli",
                        "producto multi-CLI",
                        multicli_err,
                        continuing="Continuing without the multi-CLI product.",
                    )
